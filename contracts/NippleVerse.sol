...
contract Nippleverse is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE = 69 * 10**15; // 0.069ETH Per Yaak
    uint256 private constant PRICE_PRESALE = 5 * 10**16; // 0.05ETH Per Yaak
    uint256 private constant PRICE_PREMINT = 0; // Free Per Yaak

    uint256 private constant MAX_ELEMENTS = 3333; // 3333 Nipples for Entire Collection.
    uint256 private constant MAX_ELEMENTS_PRESALE = 333; // 333 Nipples for Pre Sale.
    ...
    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying Yaak Thai ;)");
        }
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        if (_to != owner()) {
            require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of Yaak Thai");
        }
